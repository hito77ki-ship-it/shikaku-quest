...
function injectStyles(){
  ...
  s.textContent = `
.sq-toc{background:#F7FAFC;border:1px solid #E2E8F0;border-left:3px solid #8CC63F;border-radius:0 8px 8px 0;padding:16px 20px;margin:24px 0 32px;font-size:13px;}
.sq-toc-title{font-weight:700;color:#1A202C;margin-bottom:10px;font-size:13px;}
.sq-toc ol{margin:0;padding-left:20px;}
.sq-toc li{margin-bottom:5px;line-height:1.6;}
.sq-toc a{color:#6AAF2B;text-decoration:none;}
...
function buildTOC(){
  ...
  toc.className = 'sq-toc';
  toc.innerHTML = '<div class="sq-toc-title">📋 この記事の目次</div><ol>'
    + h2s.map(h=>`<li><a href="#${h.id}">${h.textContent.trim()}</a></li>`).join('')
    + '</ol>';
